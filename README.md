...
The signing script signs the executable once with hardened runtime, then signs the installer using its separate Developer ID Installer identity. It does not use `sudo`. Password dialogs during these operations normally authorize access to the signing private keys in Keychain. For fewer recurring prompts, use **Always Allow** for the expected Apple signing tool on each key, or configure that specific tool in the private key's Access Control settings in Keychain Access. This grants ongoing access while the keychain is unlocked; it is not a one-run approval. Unlocking the keychain alone does not override each key's access rules. Touch ID settings for `sudo` do not affect these signing-key prompts.

### Version and build numbers

Run `pptcrunch --version` to print the installed version, for example `pptcrunch 1.0.0.42` (release 1.0.0, build 42).

`Version.props` holds the release version. Both publish scripts derive the build number from `git rev-list --count HEAD` and pass it to MSBuild without modifying the checkout. Windows and macOS builds made from the same commit therefore receive the same version. Publishing requires Git and a full clone; shallow clones are rejected because their truncated histories can produce different counts.
...
Direct `dotnet build` commands use build number `0` unless `-p:BuildNumber=<number>` is supplied. Release artifacts should be created through `publish.bat`, `publish.sh`, or `sign-macos-builds.sh` so they use the commit-derived number. Commit the source changes before publishing when the artifact must identify that exact source revision.
...
Capture mode records **video only** from a USB HDMI capture card on Windows (Media Foundation) or macOS (AVFoundation). Run `PPTcrunch.exe capture` or `pptcrunch capture`.

On Windows, PPTcrunch uses the operating system's Media Foundation APIs to enumerate native device modes and acquire frames. This avoids stock FFmpeg's DirectShow RGB24 subtype limitation: Media Foundation RGB24 is exposed to FFmpeg as its equivalent in-memory `bgr24` layout. YUY2 (`yuyv422`), NV12, 10-bit P010 (`p010le`), and MJPEG native modes are also recognized. Mode availability still depends on the exact resolution and frame rate selected. The Win32 bindings are generated at compile time and use Windows components already in the OS, so the application remains a single-file executable; the standard downloaded FFmpeg build is still used downstream for muxing and encoding and does not require a custom patch.

The app discovers devices, frame rates, resolutions, and capture formats each time it runs. Choose a resolution first; only capture frame rates available at that resolution are then offered, including fractional rates such as 29.97/59.94. The app does not ask for or attempt to infer the HDMI source refresh rate because capture-device output timing does not reliably identify it. Exact 30 fps is the default when available, followed by 29.97 fps; otherwise the available rate closest to 30 fps is selected by default. Users can select any other device-reported rate. Tiny device interval quantization is shown with readable nominal labels (for example 30 fps for 30.00003), while the exact advertised rate is still requested. On macOS, FFmpeg selects the maximum frame rate of each advertised format range, so those are the offered rates. Screen and audio devices are excluded from the macOS menu. The selected card is addressed by its stable device ID, so a change in device index order cannot redirect recording to a different camera.

Raw color choices are sorted by spatial color detail first (RGB/4:4:4, 4:2:2, 4:2:0, then lower-color formats), then by bit precision within each group. Equivalent layouts retain their discovery order. Compressed and unrecognized formats follow because their quality cannot be inferred reliably from their names. This ranks what the output layout can preserve; it does not establish that the card delivers that much native detail. The padded RGB layouts `0rgb` and `bgr0` hold the same 8-bit color channels as RGB24 but use four bytes per pixel. Converted RGB cannot recover chroma already lost in the card/driver.

The **capture card output format** menu explains the card/driver's delivery choices. UYVY and YUYV both provide uncompressed 4:2:2 video with equivalent picture detail and data size. NV12 provides uncompressed 8-bit 4:2:0 video: the same brightness detail but less color detail, using about 25% less data at the same resolution and frame rate. P010 provides 10-bit 4:2:0 precision but twice NV12's device bandwidth. Prefer UYVY/YUYV over NV12 for fine colored text and graphics when preserving color detail matters. RGB24 provides independent 8-bit red, green and blue values at every pixel if the source supplies them; compressed formats such as MJPEG depend on the card's compression settings. These input choices are separate from the following **recording mode** menu, which explains pass-through, lossless transcoding, and lossy transcoding.

| Recording mode | Windows | macOS | File |
|---|---|---|---|
| Direct copy | Preserve the native stream delivered by Media Foundation, including RGB24/P010/MJPEG | Preserve the stream delivered by AVFoundation, normally uncompressed pixels | MKV for MJPEG/H.264/HEVC; NUT for raw/other streams |
...
Choose an output filename or accept the timestamped default. The app displays the stop instructions and waits for **Enter** before starting; **Ctrl+C** cancels at this prompt. During recording, press **q** to finish the file, then wait for the “Recording saved” message before closing the terminal. FFmpeg statistics and buffer/drop warnings stay visible. On Windows, the app measures Media Foundation sample timestamps and supplies paced frames to FFmpeg through standard input; FFmpeg assigns consecutive timestamps at the exact selected rational frame rate. No deliberate frame-rate conversion is added. The app cannot guarantee zero dropped frames if the capture backend, USB connection, encoder, or disk cannot keep up. Test sustained recording with the intended card and storage. A Media Foundation native-mode change stops the recording instead of silently substituting another input format.
...
On Windows, `PPTcrunch.exe capture --diagnose` writes `capture-diagnostics.txt` beside the executable. It includes Media Foundation's native device modes and a legacy DirectShow/FFmpeg comparison, which is useful for diagnosing driver access or subtype differences.
