...
The app discovers devices, frame rates, resolutions, and capture formats each time it runs. Choose a resolution first; only capture frame rates available at that resolution are then offered, including fractional rates such as 29.97/59.94. On macOS, FFmpeg selects the maximum frame rate of each advertised format range, so those are the offered rates. Screen and audio devices are excluded from the macOS menu. The selected card is addressed by its stable device ID, so a change in device index order cannot redirect recording to a different camera.

Enter the source's HDMI output frame rate (for example `60`, `59.94`, or `60000/1001`) to get cadence recommendations. The current capture backends do not provide PPTcrunch with reliable original HDMI timing: advertised capture rates and the short sample's timestamps describe the card's output, not necessarily its input. Check the source's HDMI refresh settings; when capturing this Mac, check the HDMI display in System Settings → Displays. A movie playing on a desktop can have a different frame rate from the HDMI signal. Leave the source rate blank if unknown, or enter `V` for variable refresh/VRR; the app will not invent a recommendation.

With a known fixed source rate, the menu prefers an exact match, then even reductions such as 60→30 or 59.94→29.97. Even multiples above the source are identified as repeated frames with no extra motion detail. Other choices carry a judder warning. The default is the source match or highest available even reduction, when possible; unknown sources retain a default near 30 fps. Tiny device interval quantization is shown with readable nominal labels (for example 30 fps for 30.00003), while the exact advertised rate is still sent to FFmpeg. 30 and 29.97 remain distinct. Even ratios reduce conversion cadence problems but cannot guarantee that the card's clocks, frame selection, USB delivery and disk/encoder keep up. Variable refresh has no single fixed divisor.
